...
public static unsafe partial class Host
{
    ...
    // Native has already gathered + read every Scripts/*.cs across all mounts and passes them here.
    // Replaces the live generation (compile new, then unload old).
    [UnmanagedCallersOnly(CallConvs = new[] { typeof(CallConvStdcall) })]
    public static int LoadScripts(FSourceFile* Sources, int Count)
    {
        try
        {
            if (Scripts == null)
            ...
            var List = new List<(string, string)>(Count < 0 ? 0 : Count);
            for (int Index = 0; Index < Count; Index++)
            {
                ref FSourceFile Source = ref Sources[Index];
                List.Add((Interop.GetString(Source.Path, Source.PathLength), Interop.GetString(Source.Text, Source.TextLength)));
            }
