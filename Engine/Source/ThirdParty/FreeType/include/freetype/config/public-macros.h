...
FT_BEGIN_HEADER
  ...
  /*
   * Lumina note: FreeType is always linked statically here, so we leave
   * FT_PUBLIC_FUNCTION_ATTRIBUTE undefined (the fallback resolves to
   * nothing). The original branch keys off `defined(DLL_IMPORT)`, which
   * Lumina sets as a value-define on every TU, so leaving it would
   * mis-trigger dllimport on consumers.
   */
